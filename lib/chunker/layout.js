...
var _renderresult = _interopRequireWildcard(require("./renderresult"));
  ...
  (0, _createClass2["default"])(Layout, [{
    ...
    value: function append(node, dest, breakToken) {
      ...
      if (node.parentNode && (0, _dom.isElement)(node.parentNode)) {
        var parent = (0, _dom.findElement)(node.parentNode, dest); // Rebuild chain
            ...
            if (tableRow) {
              ...
              if (table) {
                ...
                for (var _i = 0, _Array$from = Array.from(table.rows[0].cells); _i < _Array$from.length; _i++) {
                  var _cell = _Array$from[_i];
                  columnCount += parseInt(_cell.getAttribute("COLSPAN") || "1");
                }

                if (tableRow.cells.length !== columnCount) {
                  var previousRow = tableRow.previousSibling;
